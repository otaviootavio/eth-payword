...
import { deployEthWordMerkle, createHashchain, createMerkleTree } from "../utils/deployEthWordMerkle";
...
describe("Close Channel", function () {
  describe("Channel balance", function () {
    ...
    it("Should send one of n hashes and update the balance correctly", async function () {
      ...
      const initialBalance = await publicClient.getBalance({ address: ethWordMerkle.address });
      const bigInitialBalance = BigInt(initialBalance);

      const wordCount = 1;
      const expectedWithdraw = (bigInitialBalance * BigInt(wordCount)) / BigInt(chainSize);
      ...
      const finalBalance = await publicClient.getBalance({ address: ethWordMerkle.address });
