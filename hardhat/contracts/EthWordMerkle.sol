...
contract EthWordMerkle is ReentrancyGuard {
    ...
    function simulateCloseChannel(uint256 _amount, uint256 _random, bytes32[] calldata proof) external view returns (bool, uint256) {
    ...
    function validateChannelClosure(uint256 _amount, uint256 _random, bytes32[] calldata proof) private view returns (bool) {
        if (_amount == 0) {
            return false;
        }
